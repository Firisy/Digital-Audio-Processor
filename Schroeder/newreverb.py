...
class Freeverb2:
    def __init__(self,dry=0,damp=0.5,wet=1/3.0,roomsize=0.5,stereospread=23,gain=0.015,width=1,AP_g=0.5) -> None:    
        ...
        self.LBCFLs =[filter(self.LBCFL_b[i],self.LBCFL_a[i]) for i in range(4)]
        ...
        self.LBCFRs =[filter(self.LBCFR_b[i],self.LBCFR_a[i]) for i in range(4)]
    ...
    def process(self,input):
        ...
        output = (input * self.dry) + np.dot(outputa,self.wetmat)
        return output
    
    def update_Freeverb(self, dry=None, damp=None, wet=None, roomsize=None):
        if dry is not None:
            self.dry = dry*2
        if damp is not None:
            self.damp = damp * 0.4
